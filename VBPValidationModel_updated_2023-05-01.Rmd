...
#write.csv(Master_VBP_Rep_Comb, "./data/output/VBPMeasurementValidation/2023-04-27_Master_VBP_Rep_Comb.csv")
```
...
colnames(VBP_Rep_Comb_validation) <- VBP_Rep_Comb_validation [1,] 

# Remove the first row of data that hheaders
VBP_Rep_Comb_validation <- VBP_Rep_Comb_validation[-1,]

# 5/1/23 - Create SubMeasureID
VBP_Rep_Comb_validation$`SubMeasure ID` <- substr(VBP_Rep_Comb_validation$Measure, 1, 3)
...
#write.csv(VBP_Rep_Comb_validation, "./data/output/VBPMeasurementValidation/VBP_Rep_Comb_validation_complete_2023-04-27.csv")
```
...
Claims data was extracted from the Health Choice data warehouse for the period of ***2023-01-01 to 2022-04-27***, meaning claims adjudicated through __________ in order to be consistent with the VBP Quality Report date ranges. This data includes adjudicated claims for members of the Norther GSA, ACC health plan, in other words, Alliance Members.

The resulting solution is exported to .csv and saved as: **(./data/data_original_cat_2022-05-01_IPClaims)**. The IP Claims data is loaded to the model for analysis. In addition, a data frame was loaded that contains the HEDIS My2022 eligible ICD10 diagnoses for the FUH7 measure.
...
IPClaims <- read_xlsx("./data/data_original_cat_2023-05-01_IPClaims.xlsx", sheet = "Sheet1")
...
1.  Claims are filtered to only include Alliance Providers. There were ______ claims that were assigned to APs.
...
-   **8** VBP report details sheets were imported, merged and cleaned, creating ***Master_Rep_Comb*** with ***_________*** observations of **________** variables.

-   ***VBP_Rep_Comb_validation*** was created from the master to isolate instances of FUH7, and then select for *Member ID*, ultimately yielding **________** observations of MemberIDs.

***Claims***

-   Adjudicated claims with qualifying HEDIS My2022 inpatient service codes are queried from Health Choice data warehouse and read in as ***IPClaims***, with **_______** observations of **35** variables.

-   ***IPClaims*** is filtered to only include Alliance Providers, and only observations with a qualifying mental health diagnosis. *MemberID* and *svccode*, are selected ultimately yielding a table called ***IPClaims_validation*** with **___** observations.

***Validation Matrix***

-   The ***Validation_Matrix*** was created by reducing each of ***VBP_Rep_Comb_validation*** and ***IPClaims_validation*** to unduplicated observations of each *MemberID*, and then full outer joining them on *MemberID* yielding a table with **___** observations of **4** variables.
...
The ***VBP_Unduplicated (n=____)*** and the ***Claims_Unduplicated (n=___)*** were observed from left to right, meaning that all observations of data from ***VBP_Unduplicated*** are included, while ***Claims_Unduplicated*** are only included if matched with ***VBP_Unduplicated*** on the variable *Member ID*.

-   _ of the __ **(___%)** VBP members were validated against an eligible IP member, with **___** not matched. There are **__** false instances of eligible member events.

-   Conversely, from the perspective of IP Claims, there are **___** of **___** **(___%)** IP Claims unique MemberIDs that ***are not matched*** with an eligible VBP member. There are as many as ***____*** instances of uncounted eligible events.

***Results***

The initial results suggest possible significance. Only **___%** of the members identified by the VBP QR as eligible for the FUH7 measure actually appear in an adjudicated claim for an eligible Inpatient claim. So, even at this high level assessment there is ***limited level validation***. This discrepancy could result in mis-reporting of performance scores to the detriment of the Alliance.
...
4.  By Diagnosis:
    -   There were ___ distinct diagnoses used
...
-   ___: Matched, NonCompliant(MNC)

-   ___: Matched, Compliant (MC)

-   ___: NonMatched, NonCompliant (NMNC)

-   ___: NonMatched, Compliant (NMC)
...
-   X-squared = ___, df = 1, p-value \< ___
...
Comp_ChiSq <- Compliance %>% 
    select(Match, `Gap Status`) %>% 
  group_by(Match, `Gap Status`) %>%
  #mutate(Numerator = as.character(Numerator)) %>% 
  count() %>% 
  pivot_wider(names_from = "Gap Status",
              values_from = "n")
...
Compliance %>% 
  select(Match, `Gap Status`) %>% 
  group_by(Match, `Gap Status`) %>%
  mutate(Numerator = if_else(`Gap Status` == "OPEN", "NonCompliant", "Compliant")) %>% 
  count() %>% 
ggplot(aes(fill=Match, x=`Gap Status`, y=n)) +
  geom_col() +
